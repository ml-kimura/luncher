...
import { BatchMessageCode, formatMessageTemplate, getBatchMessageTemplate } from '../messages/templates.js';
...
import type { BatchMessageCode as BatchMessageCodeValue } from '../messages/types.js';
...
interface Dependencies {
  ...
  logger: Logger;
  channelId: string;
  announcementMessageCode: BatchMessageCodeValue;
}
...
const createDependenciesFromEnv = (): Dependencies => {
  ...
  const announcementMessageCode = BatchMessageCode.InfoDailyLunchAnnouncementPosted;
...
export const postDailyLunchAnnouncement = async (
  ...
  try {
    return await repo.withTransaction(async (client) => {
      ...
      const template = getBatchMessageTemplate(deps.announcementMessageCode, 'ja');
      const message = formatMessageTemplate(template, { lunchDate });

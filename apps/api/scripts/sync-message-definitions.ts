...
import { API_MESSAGE_DEFINITIONS } from '../src/messages/codes.js';
...
const HEADER = '# API messages configuration (source of truth)';
...
const syncMessageCodes = async () => {
  ...
  const messages: MessagesYaml['messages'] = API_MESSAGE_DEFINITIONS.map((item) => ({
    code: item.code,
    message: {
      ja: item.message.ja,
      en: item.message.en,

...
4. 対象種別に応じて削除候補を作る。
   ...
   - `prune`: `git branch -vv` を使い、`": gone]"`（例: `[origin/foo: gone]`）を含むローカルブランチ（= 追跡先リモートが削除済み）を抽出。保護ブランチは除外
...
7. 削除を実行する。
   ...
   - `prune` の場合（候補をすべて削除）:
     - `git branch -D <branch>` を順に実行（最初から強制削除）
     - 失敗した場合は失敗内容を報告する
8. 削除後の残ブランチ一覧を再表示する。
...
- `prune` 判定は `line.includes(': gone]')` で行う（`git branch -vv` の追跡先消滅表示に合わせる）。

...
> OpenAPI メタデータと API種別・成功レスポンスの規約は下記「OpenAPI メタデータと API種別」を参照する。個別エンドポイント仕様では繰り返し書かない。

## OpenAPI メタデータと API種別

設計書と OpenAPI を次で対応させる。

| 設計書（エンドポイント表） | OpenAPI（operation）                      |
| -------------------------- | ----------------------------------------- |
| **API種別** 列の値         | **`x-api-kind`** 拡張フィールド（同じ値） |

種別ごとの成功レスポンスの書き方。

- **`command`**
  - 成功時レスポンスには **`status`** を含める。
  - 処理結果を区別する必要があるときは **`result`** も含める（例: `accepted` / `no_change` / `rejected`）。単一結果でよいときは `result` は省略してよい。
- **`resource`**
  - 成功時は **リソース本体のみ** を返す。**`status` / `result` は付けない**。

エラー時の共通形式（`status: error`, `code`, `message` など）は種別に依存しない。
...
成功時の形は「OpenAPI メタデータと API種別」に従う（本節では JSON のみ例示する）。
...
`resource` では成功時レスポンスに `status` / `result` を付けず、リソース本体を返す。
